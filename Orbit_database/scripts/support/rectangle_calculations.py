...
def define_region():
    ...
    y = np.linspace(0, 1, 500)
    X, Y = np.meshgrid(x_en, y)


    y_upper = np.sqrt(2*(1.56*E0 - U_tilde(x_en, 0, mu1, mu2)))
    y_lower = np.sqrt(2*(1.6*E0 - U_tilde(x_en, 0, mu1, mu2)))
    ...
    cs = ax.contour(X, Y, region.astype(float), levels=[0.5])
    path = cs.collections[0].get_paths()[0]

    patch = PathPatch(path, facecolor="orange", alpha=0.5, edgecolor="black")
    ax.add_patch(patch)
